...
colector.on('collect', (reaction) => {
    ...
    if (!ticketsupport)
      return message.channel.send(
        "X `|` **Necesita Crear El Rango** ``Soporte de Tickets``"
  ...
  let cate = message.guild.channels.find(
	...
	message.guild
      .createChannel(supportnamech, {
		...
		await channel.send(`<@&763459995437105152>\n**El usuario ${message.author} ha creado este canal, razon: ${reason}**`).then(async (m) => {
		});
	});
});
  } else {
    message.channel.send("Este comando solo puede ser usado en el servidor de HypnoCraft NetWork")
  }
}