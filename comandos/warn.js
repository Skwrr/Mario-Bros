...
if(!warns.has(`${message.guild.id}.kicks`)){await warns.establecer(`${message.guild.id}.kicks`, 3)
}


if(args[0] === "get"){
  if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply("No puedes obtener los avisos de alguien")
...
if(args[0] === "set"){
  if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply("No tienes permisos para modificar los warns antes de la sanción")
